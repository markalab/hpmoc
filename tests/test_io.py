...
def check_load(mocfits, tablehdf5, strategy="ligo_old"):
    ...
    for m in check.meta, checktab.meta:
        for k, v in check.meta.items():
            if k != "HISTORY":
                assert good.meta[k] == v, (
                    f"meta key mismatch: {k}: {v} != "
...
def compare_new_old_ligo_io_read(infile, mask: PartialUniqSkymap = None):
    ...
    assert (old.s[iold] == new.s[inew]).all(), (
        f"old ({old}) != new ({new}) " f"for {infile}"
    )
    cov_old = uniq_minimize(old.u)[0]
    cov_new = uniq_minimize(new.u)[0]
    assert (
        cov_old == cov_new
    ).all(), f"old cov ({cov_old}) != new cov ({cov_new}) for {infile}"
...
def test_ligo_o3_bayestar_nested_fitsgz_1024():
    check_load(
        "S200316bj-1-Preliminary.fits.gz", "S200316bj-1-Preliminary.hdf5"
    )
...
def test_gracedb():
    ...
    remote = PartialUniqSkymap.read(
        "bayestar.multiorder.fits,0", graceid="S200105ae", strategy="gracedb"
    )
